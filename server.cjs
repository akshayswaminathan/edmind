...
app.post('/api/suggest-findings', async (req, res) => {
  try {
    ...
    const systemPrompt = [
      ...
      '- Labels MUST be short positive NOUN PHRASES, never sentences or clauses:',
      '  write "Cystic lesion on ultrasound", not "Ultrasound shows a cystic',
      '  lesion". They must read correctly after "supported by ..." and "the',
      '  absence of ...".',
      '- Do NOT phrase a label negatively (avoid leading "absent", "no", "normal",',
      '  "negative", "lack of"). State the positive finding and use dir "against"',
      '  instead — e.g. use {"label":"Cremasteric reflex","dir":"against"}? No:',
      '  express the finding whose PRESENCE supports/deters the dx. For a finding',
      '  that argues against when present, keep it positive (e.g. "Normal Doppler',
      '  flow" is acceptable only as a distinct against-feature; prefer pairing a',
      '  positive rule-in with its negative pole).',
      '- Do not include two features that are logical opposites of each other.',
      '- Keep true acronyms (ECG, RLQ, JVD) capitalized.',
